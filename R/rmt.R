...
#' @return A list containing: The restricted mean survival in favor of the treatment arm, the components of the treatment effect, and the maximum time used in comparisons.


# ---------------------------------------------------
# Restricted mean survival in favor of treatment
# ---------------------------------------------------
RMT <- function(m,Time,Delta,trt,time_restriction,dist_state0,dist_state1,unique_event_times0,unique_event_times1,nunique_event_times0,nunique_event_times1,maxfollow0,maxfollow1,max_time_inc) {
#  cat("Starting RMT", "\n")
  ...
  while (j < nunique) {
    ...
    # Add wintime
      ...
      #     if (j==138) {cat("new_dist1[event_num,j]=",new_dist1[event_num,j],"\n")}
        rmt_time <- rmt_time + new_dist1[event_num,j] * sum(new_dist0[((event_num+1):(m+1)),j]) * (untimes[j+1]-untimes[j])
    ...
    # Subtract Wintime
      ...
      #     if (j==138) {cat("new_dist0[,j]=",new_dist0[,j],"\n")}
      #     if (j==138) {cat("new_dist1[event_num,j]=",new_dist1[event_num,j],"\n")}
        rmt_time <- rmt_time - new_dist1[event_num,j] * sum(new_dist0[(1:(event_num-1)),j]) * (untimes[j+1]-untimes[j])
        components[event_num-1] <- components[event_num-1] - new_dist1[event_num,j] * sum(new_dist0[(1:(event_num-1)),j]) * (untimes[j+1]-untimes[j])
      }
    } else {
      if (untimes[j] <= time_restriction) {
        #        cat("j=",j,"\n")
        # Add wintime
        for (event_num in 1:m) {
          rmt_time <- rmt_time + new_dist1[event_num,j] * sum(new_dist0[((event_num+1):(m+1)),j]) * (time_restriction-untimes[j])
          for (k in (event_num+1):(m+1)) {
            components[k-1] <- components[k-1] + new_dist1[event_num,j] * new_dist0[k,j] * (time_restriction-untimes[j])
          }
        }
        # Subtract wintime
        for (event_num in 2:(m+1)) {
          rmt_time <- rmt_time - new_dist1[event_num,j] * sum(new_dist0[(1:(event_num-1)),j]) * (time_restriction-untimes[j])
          components[event_num-1] <- components[event_num-1] - new_dist1[event_num,j] * sum(new_dist0[(1:(event_num-1)),j]) * (time_restriction-untimes[j])
        }
        #        cat("updated RMT=",rmt_time,"\n")
      }
    }
    #   cat('after losses:rmt_time=',rmt_time,'\n')
    j=j+1
  }

  # cat('-------------------------------------','\n')
  # cat('-------------------------------------','\n')
  # cat('Final rmt_time=',rmt_time,'\n')
  # cat('-------------------------------------','\n')

  return(list(rmt_time,components))
}
