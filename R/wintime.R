...
#' Rows should represent events and columns should represent participants. Event rows should be in increasing order of clinical severity.
#' @param trt A numeric vector of length 'n' containing treatment arm indicators (1 for treatment, 0 for control).
...
#' @param max_time_inc Optional. Maximal time increment for updating multi-state distribution when parametric exponential extension models are used (EWT, EWTP, EWTPR, RMT, REWTP, REWTPR only).
...
#' a vector of length 'm' with the variance of the components.
...
# ---------------------------------------------
wintime <- function(type,Time,Delta,trt,cov = NULL,model = NULL,resample = NULL,resample_num = 0,time_restriction = NA,seed = NA,max_time_inc = NA) {
  #cat('Wintime called with model=',model,'trt=',trt,'\n')
  # -------------------------------
  # Validate inputs
  # -------------------------------
  type <- tolower(type)
  ...
  type <- tolower(type)
  if (is.null(model)) {
    if (type %in% c("ewt","rmt","ewtr","max","ewtpr","rewtpr","ewtp","rewtp")) {
      model <- "markov"
    }
    else {
      model <- NULL
  ...
  if (is.null(resample)) {
    if (type == "ewt" || type == "rmt" || type == "ewtpr" || type == "rewtpr" || type == "wtr" || type == "rwtr" || type == "pwt" || type == "max" || type == "rpwt") {
      resample <- "boot"
    }
    else {
      resample <- NULL
...
#  cat('model=',model,'\n')
  ...
  if (!is.null(z)) {
    ...
    # cat('untimes0[1]=',untimes0[1],'\n')
    # cat('nuntimes1=',nuntimes1,'\n')
    # cat('untimes1[1]=',untimes1[1],'\n')
    # cat('nuntimes2=',nuntimes2,'\n')
    # cat('untimes2[1]=',untimes2[1],'\n')
  ...
  if (type == "ewt") {
    obs_data <- EWT(m,Time,Delta,trt,dist_state0,dist_state1,untimes0,untimes1,nuntimes0,nuntimes1,max_follow0,max_follow1,max_time_inc)
    #max_time <- obs_data[[3]]
  }
  else if (type == "ewtr") {
    ...
    obs_data <- EWTP(n,m,nuntimes2,max_follow2,untimes2,Time,Delta,dist_state2,markov_ind,cov,trt,max_time_inc)
    #max_time <- obs_data[[6]]
  }
  else if (type == "rewtp") {
    ...
    obs_data <- REWTP(n,m,nuntimes2,max_follow2,untimes2,Time,Delta,dist_state2,markov_ind,cov,trt,time_restriction,max_time_inc)
    #max_time <- obs_data[[6]]
  }
  else if (type == "ewtpr") {
    obs_data <- EWTPR(n,m,nuntimes2,max_follow2,untimes2,Time,Delta,dist_state2,markov_ind,cov,trt,trans_prob2,nuntimes1,max_follow1,untimes1,dist_state1,trans_prob1,nuntimes0,max_follow0,untimes0,dist_state0,trans_prob0,max_time_inc)
    #max_time <- obs_data[[6]]
    #cat('obs_data=','\n')
    #print(obs_data)
  }
    ...
    obs_data <- REWTPR(n,m,nuntimes2,max_follow2,untimes2,Time,Delta,dist_state2,markov_ind,cov,trt,trans_prob2,time_restriction,nuntimes1,max_follow1,untimes1,dist_state1,trans_prob1,nuntimes0,max_follow0,untimes0,dist_state0,trans_prob0,max_time_inc)
    #max_time <- obs_data[[6]]
  }
  else if (type == "rmt") {
    if (is.na(time_restriction)) {
    ...
    obs_data <- RMT(m,Time,Delta,trt,time_restriction,dist_state0,dist_state1,untimes0,untimes1,nuntimes0,nuntimes1,max_follow0,max_follow1,max_time_inc)
   # max_time <- obs_data[[3]]
  }
  else if (type == "wtr") {
...
#    z_rewtpr <- REWTPR(n,m,nuntimes2,max_follow2,untimes2,Time,Delta,dist_state2,markov_ind,cov,trt,comkm,trans_prob,time_restriction)[[3]]
  ...
  if (!is.null(resample)) {
    ...
    if (resample == "boot" || resample == "bootstrap") {
      ...
      temp <- bootstrap(type,time_restriction,model,n,m,Time,Delta,trt,cov,z_ewtr,z_comp,resample_num,seed,max_time_inc)
    ...
    # Run type function on permuted data
      ...
      temp <- perm(type,time_restriction,model,n,m,Time,Delta,trt,cov,z_ewtr,z_comp,resample_num,seed,max_time_inc)
      resample_data <- temp[[1]]
      if (type %in% c("ewtr","ewtp","rewtp","ewtpr","rewtpr","pwt","rpwt","rmt","ewt")) {
        resample_components <- temp[[2]]
      }
...
#  cat('warnings','\n')
  ...
  if (type == "max" && (model == "km" || resample %in% c("perm","perms","permutation","permutations"))) {
    ...
    return(list(trt_effect = data, resample_data = resample_data, message = message, variance = variance, p = p, wins = wins, losses = losses, components=components, components_var=components_var))
}
